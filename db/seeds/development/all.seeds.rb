...
question1 = FactoryBot.create(:question, text: "Сколько трехзначных чисел можно составить из 4 цифр: 1, 2, 3, 4?", unit: unit)
question2 = FactoryBot.create(:question, text: "Сколько двузначных чисел, в которых цифры не повторяются, можно составить из 4 цифр: 1, 2, 3, 4?", unit: unit)
question3 = FactoryBot.create(:question, text: "Сколько пятизначных чисел, в которых цифры не повторяются, можно составить из 6 цифр: 1, 2, 3, 4, 5, 6 ?", unit: unit)
question4 = FactoryBot.create(:question, text: "Сколько двузначных чисел можно составить из 5 цифр: 1, 2, 3, 4, 5?", unit: unit)
question5 = FactoryBot.create(:question, text: "Сколько трехэлементных подмножеств, различающихся хотя бы одним элементом друг от друга и без учета порядка в подмножестве, можно составить из 6 цифр: 1, 2, 3, 4, 5, 6?", unit: unit)
question6 = FactoryBot.create(:question, text: "Сколько будет костей в игре домино, если использовать, только четыре цифры 1, 2, 3, 4?", unit: unit)
